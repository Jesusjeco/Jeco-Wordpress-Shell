...
class Ai1wmde_Export_Incremental_Themes {

	public static function execute( $params, Ai1wmde_Dropbox_Client $dropbox = null ) {

		// Set progress
		Ai1wm_Status::info( __( 'Preparing incremental theme files...', AI1WMDE_PLUGIN_NAME ) );
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_themes_list_path( $params ), 'wb' ) ) ) {
			try {
				$dropbox->get_file( $incremental_list, sprintf( '%s/incremental-backups/incremental.themes.list', ai1wmde_get_folder_path() ) );
		...
		// Set progress
		Ai1wm_Status::info( __( 'Done preparing incremental theme files.', AI1WMDE_PLUGIN_NAME ) );
